...
EXTRACT_SPEC = os.environ.get("FACTLAYER_EXTRACT", "gemini:gemini-3.5-flash-lite")
JUDGE_SPEC = os.environ.get("FACTLAYER_JUDGE", "gemini:gemini-2.5-flash")
...
_HARD_QUOTA = ("perday", "per day", "daily", "generate_requests_per_model_per_day",
               "free_tier_requests", "limit: 0")


def _quota_is_exhausted(detail):
    """True when a 429 names a limit that will not clear during this run."""
    low = detail.lower()
    return any(m in low for m in _HARD_QUOTA)
